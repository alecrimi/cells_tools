...
legnth = 1000; %Length Ro
width = 700;

%Load file
fname = 'USExpII_580kHz_40%e_30min_LTT-middle_8%AA_ck8-18-649P_647ex_Quadruple_lp10_rightL_1-6x_15umStep(431).tif';
disp('#######################################')
disp('Loading images');
info = imfinfo(fname); 
num_images =   170;%numel(info);
...
%n_detected_cells = max(max(L));
    ...
    for jj = 1 : n_detected_cells
        ...
        [r c v] = find(L == jj);
        mean_inside = mean(mean(Im_test_cropped(r,c,nn))); %mean(mean(overall_imm(r,c)));%Im_test_cropped(round(mean(r)),round(mean(c)),nn);%
        center_x = mean(c);
        plot(rect(2)+center_x, mean_inside,'b*','MarkerSize',3);
    end
end
hold off
