...
results = zeros(length(list)/2,2);
result_count = 1;
 
for kk = 1 : 2 :  length(list)
    
    [  n_inclusions, mean_int ] =  p62( list(kk+1).name, list(kk).name);
    
    results(result_count,1)  = n_inclusions; 
    results(result_count,2)  = mean_int; 
...
csvwrite('results_feautes_p62.csv',results);
  
% Average per well field
results_averaged = zeros(length(list)/18,2);
result_count = 1;
for jj = 1 : 9:  length(list)/2
    results_averaged(result_count,:)  = mean( results(jj:jj+8,:)  );
    result_count = result_count +1;
end
...
if fid>0
     ...
     fprintf(fid,'n_inclusions,mean_int\n');
...
plot_data_p62('avg_results_features.csv')
